(() => {
  ...
  // ---------- Load JSON (moved to external file) ----------
  async function loadData() {
    try {
      ...
      document.body.innerHTML = '<pre class="json-out">' +
        JSON.stringify({ error: "Failed to load repoversion.json", detail: String(e) }, null, 2) +
        '</pre>';
  ...
  function handleMachineEndpoints(data) {
    ...
    if (format === "code") {
      if (!appId){
        ...
        document.body.innerHTML = '<pre class="json-out">'+lines+'</pre>'; document.title="versions.txt"; return true;
      } else {
        const app = apps.find(a=>a.id===appId);
        const code = String(app ? (pickLatest(app)?.code ?? 0) : 0);
        document.body.innerHTML = '<pre class="json-out">'+code+'</pre>'; document.title=`${appId}.code`; return true;
      }
    }
...
download=${L.download??""}\n`;
      ...
      if (appId){
        ...
        document.body.innerHTML = '<pre class="json-out">'+out+'</pre>'; document.title=`${appId}.${format}`; return true;
      }
      if (latestOnly){
        const out = apps.map(a=>`${a.id} ${(pickLatest(a)?.code??0)} ${(pickLatest(a)?.version??"")}`).join("\n");
        document.body.innerHTML = '<pre class="json-out">'+out+'</pre>'; document.title="latest.txt"; return true;
      }
      const out = apps.map(block).join("\n");
      document.body.innerHTML = '<pre class="json-out">'+out+'</pre>'; document.title=`versions.${format}`; return true;
    }
    ...
    if (format === "json") {
      if (latestOnly){
        ...
        document.body.innerHTML = '<pre class="json-out">'+JSON.stringify(out,null,2)+'</pre>'; document.title="versions.json"; return true;
      }
      if (appId){
        const app = apps.find(a=>a.id===appId);
        if (!app){
        ...
        document.body.innerHTML = '<pre class="json-out">'+JSON.stringify(out,null,2)+'</pre>'; document.title=`${app.id}.json`; return true;
      }
      document.body.innerHTML = '<pre class="json-out">'+JSON.stringify(data,null,2)+'</pre>'; document.title="versions.json"; return true;
    }
  ...
  function initInteractive(data){
    ...
    // NEW: show total apps count in the header chip
    ...
    function buildPicker(appIds){
      ...
      appIds.forEach(({id,name})=>{
        ...
        const choose = ()=>{ setPicker(id, name||id); closePicker(); updateEndpointBoxes(); };
    ...
    document.addEventListener("click", async (e)=>{
      ...
      if (mini){
        ...
        if (mini.getAttribute("data-copy")==="endpoint"){
          const url = new URL(location.href); url.search = val;
          text = url.toString();
        }
        try{
          await navigator.clipboard.writeText(String(text));
          const t = mini.textContent; mini.textContent = "Copied!"; setTimeout(()=>mini.textContent=t,900);
        }catch{
          alert(String(text));
        }
      }
  ...
  (async function boot(){
    ...
    // If ?format=* is present, render machine output and stop
    if (handleMachineEndpoints(data)) return;
    // Otherwise render interactive UI
